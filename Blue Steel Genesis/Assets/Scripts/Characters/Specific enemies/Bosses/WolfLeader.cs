...
{
    private int remainingSummons = 2; // Ñêîëüêî ðàç ìîæíî èñïîëüçîâàòü ïðèçûâ (êàæäûé ðàç ïûòàåòñÿ ïðèçâàòü  äî 2 âîëêîâ)
    ...
    {
        if (remainingSummons <= 0) { targetPos = default; return false; }
    ...
    // Äâèæåíèå (ðàíüøå áûëî äëÿ ïðèçûâà, òåïåðü èíäåêñ 3)
    protected override bool TryGetTargetForThree(out Vector3Int targetPos)
    {
        targetPos = Position.LeftBottom;
        var moveRange = priorityModules[3].getCellsInRange(Position).Concat(Position).ToHashSet();
        var path = Navigation.Dijkstra.getPath(Position,
            getEnemies().SelectMany(e => e.Position.NeighborPositions()),
